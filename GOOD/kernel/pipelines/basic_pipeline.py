...
class Pipeline:
    ...
    def train(self):
        ...
        for epoch in range(self.config.train.ctn_epoch, self.config.train.max_epoch):
            ...
            for stat, split in zip([epoch_train_stat, id_val_stat], ["train", "valid"]):
                ...
                node_gt = torch.cat(stat["node_gt"]) if "node_gt" in stat and len(stat["node_gt"]) > 0 else None
                ...
                bkg_att_weights = torch.cat((bkg_att_weights, torch.zeros_like(bkg_att_weights[:1])),dim=0)
                #signal_att_weights = torch.cat((signal_att_weights, torch.zeros((1))), dim=0)
                signal_att_weights = torch.cat((signal_att_weights, torch.zeros_like(signal_att_weights[:1])),dim=0)
    ...
    def generate_binary_explanations(self, is_weight, thrs, splits, convert_to_nx, is_node_expl):
        # GSATGNNs_MODIFIED returns node-level explanations for BAColorGVIsolated.
        # GOOD may infer edge-level explanation from the inherited GSAT config,
        # so force node-level mode for this modified model.
    ...
    def compute_metric(
        ...
        if len(eval_samples) <= 1:
            print(f"\nToo few intervened samples, skipping this metric")
