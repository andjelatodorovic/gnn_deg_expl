# from https://github.com/bknyaz/graph_attention_pool/blob/master/extract_superpixels.py
import numpy as np
import random
import os
import scipy
...
def parse_args():
    ...
    parser.add_argument('-d', '--data_dir', type=str, default='./storage/datasets/tmp/MNIST', help='path to the dataset')
...
def process_image(params):
    ...
    while n_sp_extracted > args.n_sp:
        ...
        '''
            Track of changes wrt the original code from

            Original line:
                'slic(img, n_segments=n_sp_query, compactness=args.compactness, multichannel=len(img.shape) > 2)'
            Replaced with:
                'slic(img, n_segments=n_sp_query, compactness=args.compactness, channel_axis=None, start_label=0)'
            
            To account for changes in the package version
        '''
        superpixels = slic(img, n_segments=n_sp_query, compactness=args.compactness, channel_axis=None, start_label=0)
...
'''
    How to call this script (https://github.com/bknyaz/graph_attention_pool/blob/master/scripts/prepare_data.sh):

    'for split in train test; do python extract_superpixels.py -s $split -o ./storage/datasets/MNIST/raw/ --seed 111; done'
'''
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        assert False, "multichannel parameter disabled in this version of scikit-image. Assumng greyscale images"
